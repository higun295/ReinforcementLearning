...
for lr in learning_rates:
    ...
    print(f"학습률 {lr}의 최종 손실: {final_loss:.6f}")
...
for result in results:
    plt.plot(range(0, iters, 1000), result['loss_history'],
             marker='o', linestyle='-', label=f"학습률 {result['lr']} (최종 손실: {result['final_loss']:.4f})")
...
plt.bar(range(len(lrs)), final_losses, tick_label=[str(lr) for lr in lrs])
plt.xlabel('학습률')
plt.ylabel('최종 손실')
plt.title('학습률별 최종 손실 비교')
...
for result in results:
    plt.plot(t, result['predictions'], linewidth=2, label=f"학습률 {result['lr']}")

plt.xlabel('x')
plt.ylabel('y')
plt.title('학습률별 예측 결과 비교')
plt.legend()
plt.grid(True)
...
for i, result in enumerate(results):
    ...
    plt.scatter(x, y, s=10, alpha=0.3, label='학습 데이터')
    plt.plot(t_smooth, np.sin(4 * np.pi * t_smooth), 'g-', linewidth=1, alpha=0.7, label='실제 sin(4πx)')
    plt.plot(t, result['predictions'], 'r-', linewidth=2, label=f"학습률 {result['lr']}")

    plt.xlabel('x')
    plt.ylabel('y')
    plt.title(f"학습률 {result['lr']} (손실: {result['final_loss']:.4f})")
    plt.legend()
    plt.grid(True)
...
# 최적 학습률 찾기
best_lr_idx = np.argmin([r['final_loss'] for r in results])
best_result = results[best_lr_idx]
print(f"\n최적 학습률: {best_result['lr']}")
